...
Tracker::Tracker(std::string type) :
    ...
    slizePub(nh_.advertise<sensor_msgs::PointCloud2> ("/tracking/velodyne_slize_2",1)),
    personPub(nh_.advertise<sensor_msgs::PointCloud2> ("/tracking/velodyne_person_2",1)),
...
//        Segmentation seg(6000, 500, 0.30);			// Velodyne mounted on pioneer
        // cluster tolerance BUG!!! try with 3rd argument 0???
...
//		seg.cluster(personPCL);
        ...
        GPB_Segmentation seg(6000, 500, 0.30);			// Velodyne mounted on pioneer
        seg.setInputCloud(inPCLptr);
        seg.cutOutSector(pos.theta, fov, filteredPCLptr);
        ros::Time start = ros::Time::now();
        seg.removeFloor(filteredPCLptr);
        seg.removeFloorGPB(filteredPCLptr);

        seg.cluster(personPCL);
        ros::Duration elapsed = ros::Time::now() - start;

        ROS_INFO("Took %lld nsec", elapsed.toNSec());
